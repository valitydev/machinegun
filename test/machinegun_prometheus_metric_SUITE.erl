...
events_sink_kafka_sent_test(_C) ->
    ...
    _ = maps:fold(
        fun(DurationMs, BucketIdx, {Counter, BucketAcc}) ->
            ...
            {BucketsHits, _} =
                prometheus_histogram:value(mg_events_sink_kafka_sent_duration_seconds, [?NS, Name, encode]),
            {BucketsHits, _} =
                prometheus_histogram:value(mg_events_sink_kafka_sent_duration_seconds, [?NS, Name, send]),
            BucketHit = lists:nth(BucketIdx, BucketsHits),
            %% Check that bucket under index BucketIdx received one hit
